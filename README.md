# 블랙잭
## 구현할 기능 목록
- [x] 1. 게임에 참여할 사람을 사용자에게 입력받음 
  - [x] 1.1. 입력받은 문자를 쉼표로 구분해 각각 구분
  - [x] 1.2. 이름이 0자일 경우 재입력받기
  - [x] 1.3. 최대 플레이어 수(7명)보다 많을 경우 재입력받기
- [x] 2. 플레이어 별 배팅 금액을 입력받기
  - [x] 2.1. 배팅 금액이 0원 이하일 경우 재입력받기
- [x] 3. 총 52장의 카드를 딜러와 플레이어에게 각각 2장씩 순서대로 나누어주기
- [x] 4. 딜러의 카드 중 한 장의 카드를 제외한 카드를 모두 공개하여 출력
  - [x] 4.1. 딜러의 카드가 블랙잭(2장의 카드를 받고 21점)일 경우 그 즉시 게임을 종료 - 7로 이동
  - [x] 4.2. 플레이어도 블랙잭이면 배팅액을 돌려받으며 아닐 경우 배팅액을 잃음
- [x] 5. 입력한 플레이어 순으로 카드 더받기 여부 결정 여부를 입력받음
  - [x] 5.1. 플레이어가 카드를 더 받을 경우 추가로 받은 카드를 포함하여 플레이어가 가진 카드를 모두 출력
  - [x] 5.2. 21 이하일 경우, 결정 여부를 추가로 입력받으며 카드 합이 21 초과일 경우 종료 
- [x] 6. 플레이어의 결정이 모두 끝나면 딜러의 카드가 16점 이하일 경우 딜러 더 받기, 17 이상일 경우 종료시키기
- [x] 7. 모든 결정 종료 후 딜러를 포함한 각 참여자의 모든 카드를 출력하고 최종 점수를 표시
  - [x] 7.1. ace 카드를 각 참여자에게 유리한 방향으로 조정
- [x] 8. 점수를 비교하여 딜러보다 점수가 높으면 배팅한 금액을 얻으며 낮을 경우 배팅액을 잃음, 비길 경우 배팅액을 돌려받음
  - [x] 8.1. 플레이어가 블랙잭으로 이길 경우, 배팅한 금액의 1.5배를 얻음
  - [x] 8.2. 딜러가 21점을 초과할 경우, 그 때 동안 참여 중인 전원에게 배팅한 금액을 지급함
- [x] 9. 버스트를 포함한 모든 참여자의 최종 수익을 계산하여 출력
## 기능 요구 사항
