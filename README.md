## 기능
- 게임 시작시, 56장의 카드 더미가 생성된다. [X]
- 각 카드는 symbol로 스페이드, 다이아몬드, 하트, 클로버 중 하나를 갖는다. [X]
- 각 카드는 type으로 1~10, J, Q, K, A 중 하나를 갖는다. [X]
- 딜러는 쉼표로 구분된 이름을 입력받아야한다. [X]
    - 이름이 10자를 넘기지 않는지, 영어 알파벳 이외의 문자가 쓰이지 않았는지 검증할 수 있어야한다. [X]
