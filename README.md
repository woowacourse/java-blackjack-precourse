...
- 게임 진행 메서드의 흐름은 다음과 같다.
  ...
  - (완료) 플레이어 순회가 끝나면, 딜러 객체에 대해 업데이트(패 드로우)를 수행한다.
...
- (완료) 카드팩토리 클래스 : 트럼프카드 한 벌을 생성하는 메서드를 가지고 있다. 정적으로 구현된다.
  ...
  - 덱 생성 메서드
- (완료) 덱 클래스 : 트럼프카드 한 벌을 보관하면서 카드를 뽑는 메서드를 포함하는 덱 객체이다.
  - 카드팩토리를 통한 덱 생성
  - 카드 뽑기 메서드
...
- 딜러 클래스 : 플레이어의 기능들을 포함하는 클래스. 기능 목록은 다음과 같다.
  - (완료) 카드 리스트(즉, 플레이어가 손에 쥔 패 목록)를 가지고 있다.
  - (완료) 메서드를 통해 카드를 뽑는 동작을 취할 수 있다.
  - (완료) 현재 가진 카드의 점수 합을 계산할 수 있다.
  - (완료) 현재 가진 패를 출력할 수 있다.
  - (완료) 게임 오버 여부(21점을 넘었는지, 다른 사람의 점수보다 높은지 등)를 인자를 통해 판단할 수 있다.
- (완료) 카드를 추가로 뽑을 지 확인해주는 메서드를 가진다. 패의 상황에 따라 자동으로 실행된다.
- 플레이어 클래스 : 딜러를 상속하여 추가적인 기능을 구현한 클래스. 기능 목록은 다음과 같다.
  - (완료) 카드를 추가로 뽑을 지 물어보도록 뽑기 메서드를 오버라이딩.
  - (완료) 딜러의 점수를 바탕으로 얻는 배당금을 구할 수 있다.
...
- (완료) 배팅 금액 입력 예외처리
- (완료) 추가 드로우 입력(플레이어 클래스) 예외처리
- (완료) 더 뽑을 카드가 없는 경우 : 새 덱을 세팅해 주는 식으로 구현하도록 수정
...
- 드로우할 카드가 없는 경우
  - 만약 카드가 다 떨어질(== 인덱스가 범위를 초과할) 경우, 새 덱을 세팅한다.
  - **기존에는 예외처리로 구현하였으나, 새 덱을 세팅하는 식으로 로직을 변경하였다.**
- 생성자 호출 문제
  - 생성자가 호출되지 않는 것을 고려한 동적 객체의 경우, 생성자 호출 시 AssertionError를 호출하도록 한다.

**요구사항 외의 문제**

- 배팅의 우선순위 문제

  - 만약 첫 두 패의 점수합이 블랙잭(21)일때, 딜러가 21을 넘겨서 졌다면 첫 두 패의 블랙잭에 따른 상금(1.5배)를 받아야 하는가? 

    아니면 딜러의 패배에 따른 상금(2배)를 받아야 하는가?

    **일단 첫 두 패가 블랙잭이면 게임에서 나간다고 가정하고 이후 연산에서 제외하도록 하였다.**

  - 무승부일 경우 돈을 잃는가?

    문제에는 "둘 다 블랙잭이면 돈을 돌려준다"라고만 되어있지 무승부일때 돌려주는지 여부는 써있지 않다.

    **일단 무승부여도 돌려주도록 설계하였다.**
...
- 리팩토링
  ...
  - private/protected/public 정확한지 확인할 것!

  - 객체 내 추가 예외처리 필요 여부 확인할 것!
