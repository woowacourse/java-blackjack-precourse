...
14. (Business / **구현완료**) 카드를 더 받는지 여부에 대한 입력이 유효한지 확인하는 기능.

    - (예외) 입력이 "y" 또는 "n"가 아닌 경우 확인.

15. (Business / **구현완료**) 카드를 더 받을 경우, 카드를 거절하기 전까지 계속 물어보는 기능.

16. (Business) 모든 Player가 더 이상 카드를 받지 않는 경우, 규칙에 따라 Dealer에게 카드를 주는 기능.
...
17. (UI / **구현완료**) Dealer가 카드를 더 받아야 하는 상황인지를 출력하는 기능.

18. (Business) Dealer가 카드를 모두 받은 다음 Dealer와 모든 Player의 결과를 계산하는 기능.

    - (예외) Player 카드 중에 Ace가 존재하는 경우, 11로 계산했을때 버스트(합이 21을 초과)가 아닐 경우는 11로, 버스트(합이 21을 초과)일 경우는 1로 계산하도록 처리.

19. (UI / **구현완료**) Dealer와 Player의 최종 결과를 출력하는 기능.

20. (Business) 게임 결과에 따라 Dealer와 각 Player의 수익을 계산하는 기능.

21. (Business) Dealer와 Player의 가장 처음 뽑은 2개의 카드가 블랙잭(합이 21)인 경우를 확인하는 기능.
