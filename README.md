...
1. 플레이어들의 추가 카드를 지급한다.
    ...
    * [유의사항] 카드를 받는 도중 21을 넘길 경우 버스트임을 알리고 카드 지급을 중단한다.
    * 모든 플레이어의 추가 카드 지급을 마치면 마친다.
    * 처음 받은 두 장의 카드의 점수가 21이면 블랙잭이므로 추가 카드를 지급받지 않는다.
    
1. 딜러의 카드 상황을 확인하고 추가지급한다.
    * 딜러의 점수 합이 16이하면 추가로 카드를 지급한다.
    * [유의사항] 추가로 받았는데도 16이하라면 16이상이 될 때까지 한 장씩 계속 지급받는다.
    * Ace는 기본 11로 계산한다.
    * [유의사항] 추가로 받았을 때 버스트이고, Ace를 보유한 경우 Ace를 1로 계산한 뒤 16이하인지 다시 체크한다.
...
1. 배팅 금액을 계산하여 최종 수익을 출력한다.
	* 플레이어와 딜러 중 21에 가까운 사람이 승리한다.
	* 딜러가 21을 초과할 경우 플레이어의 카드와 상관없이 플레이어에게 배팅금액을 지불한다.
    * 플레이어가 21을 초과할 경우 배팅 금액을 모두 딜러에게 지불한다.
    * 처음 두 장의 카드 합이 21일 경우(블랙잭) 베팅 금액의 1.5배를 딜러에게 받는다.
    * 플레이어와 딜러 모두 블랙잭일 경우 배팅 금액을 돌려받는다.
