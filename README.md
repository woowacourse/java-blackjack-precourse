...
10. 딜러가 카드의 합이 17점 이상이 되기까지 카드를 반복해서 받는 기능 v
11. 게임이 종료되면 베팅 결과를 산출하는 기능 v

# 3주차 피드백
 - 시작 단계에서 너무 완벽한 설계하는 것을 경계하라.
 - 상황(context)에 맞는 설계와 구현 방법을 찾아라.
 - 원시 타입과 문자열을 포장하라.
 - 적절한 자료구조(collection)을 활용하라.
 - 일급 자료구조(collection)을 사용한다.
    > 하나의 Collection을 객체로 포장한다.
 - 객체에 메시지를 보내라.
 - Enum 도 객체와 같다. 값을 꺼내지(get) 말고 메시지를 보내 로직을 구현한다.
 - git 커밋 메시지를 잘 쓰려고 노력해야 하는 이유와 잘 작성하는 방법에 학습하는 시간을 갖질 것을 추천한다.
