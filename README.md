...
6. (Business / **구현완료**) 배팅 금액이 유효한지 확인하는 기능.
   ...
   - ~~(예외) 추후에 수익을 얻게 될 때, 받을 수 있는 금액인지 확인.~~ - Player 객체의 배팅 금액이 double형이라 상관 없음.

     - ~~즉, 짝수인지 확인. 홀수인 경우에 블랙잭이면 1.5배를 받게 되는데, 존재하지 않는 금액 단위로 받을 수 있음.~~

     - ~~예시 : 15원을 배팅하면 수익이 22.5원이 되는데 0.5원이라는 것은 존재하지 않음.~~
...
12. (UI / **구현완료**) Dealer는 처음 받은 1장, 나머지 Player들은 2장씩 출력하는 기능.

    - (구현) Dealer는 처음 받은 1장을 출력하도록 구현.

13. (UI / **구현완료**) 각 Player별로 카드를 더 받을지 여부를 입력받는 기능.

14. (Business) 카드를 더 받는지 여부에 대한 입력이 유효한지 확인하는 기능.

    - (예외) 입력이 "y" 또는 "n"가 아닌 경우 확인.

15. (Business) 모든 Player가 더 이상 카드를 받지 않는 경우, 규칙에 따라 Dealer에게 카드를 주는 기능.
...
16. (UI / **구현완료**) Dealer가 카드를 더 받아야 하는 상황인지를 출력하는 기능.

17. (Business) Dealer가 카드를 모두 받은 다음 Dealer와 모든 Player의 결과를 계산하는 기능.

    - (예외) Player 카드 중에 Ace가 존재하는 경우, 11로 계산했을때 버스트(합이 21을 초과)가 아닐 경우는 11로, 버스트(합이 21을 초과)일 경우는 1로 계산하도록 처리.

18. (UI / **구현완료**) Dealer와 Player의 최종 결과를 출력하는 기능.

19. (Business) 게임 결과에 따라 Dealer와 각 Player의 수익을 계산하는 기능.

20. (Business) Dealer와 Player의 가장 처음 뽑은 2개의 카드가 블랙잭(합이 21)인 경우를 확인하는 기능.

21. (Business) 최종 게임 결과(수익)을 계산하는 기능.
...
22. (UI / **구현완료**) 최종 게임 결과(수익)을 출력하는 기능. 
