...
3. 카드들을 섞는다.
4. 덱은 게임 시작 후 딜러를 포함한 모든 게임 참여자(유저)들에게 두 장의 카드를 지급한다.
5. 딜러를 포함한 모든 참여자의 카드 상황을 보여준다. 이 때, 딜러는 첫 번째 카드만 공개한다. 플레이어들은 모든 카드를 공개한다. 
6. 각 플레이어는 카드를 확정한다. 즉, 플레이어가 카드를 더이상 지급받지 않는다고 선언하거나 버스트할 때까지 덱으로부터 카드를 지급받는다.
7. 딜러는 플레이어들의 카드가 모두 확정되면, 규칙에 따라 덱으로부터 카드를 추가로 지급받거나 지급받지 않는다.
8. 딜러와 플레이어들의 확정된 카드들과 카드 숫자 총합을 출력한다.
9. 각 참여자의 최종 결과(수익)을 출력한다.