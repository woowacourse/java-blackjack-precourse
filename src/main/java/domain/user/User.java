...
public class User {
     ...
     */
    protected static final int ACE_CARD_USED_ONE = 0;

    /**
     * ACE card를 11로 사용할 경우, 추가로 점수에 더해야 할 값으로 사용할 상수.
     */
    protected static final int ACE_CARD_USED_ELEVEN = 10;

    /**
     * 블랙잭(처음 2장의 care의 총합이 21)인지 확인할 때 카드의 장수를 확인하기 위해 사용할 상수.
     */
    protected static final int NUMBER_OF_CARDS_TO_BLACK_JACK = 2;
    ...
    public int getScore() {
        ...
        return calculateIfAceCardExist(score);
    }
     ...
     */
    protected int calculateIfAceCardExist(int score) {
        for (Card card : cards) {
            score += addAdditionalScoreIfAceCard(score, card);
        }
        return score;
    }
     ...
     */
    protected int addAdditionalScoreIfAceCard(int totalScore, Card card) {
        if (card.isAceCard()) {
            return useAceCardOneOrEleven(totalScore);
        }
        return NOT_ACE_CARD;
    }
     ...
     */
    protected int useAceCardOneOrEleven(int totalScore) {
        if ((totalScore + ACE_CARD_USED_ELEVEN) >= BUST_SCORE) {
            return ACE_CARD_USED_ONE;
        }
        return ACE_CARD_USED_ELEVEN;
    }
    ...
    public boolean isBlackJack() {
        return ((cards.size() == NUMBER_OF_CARDS_TO_BLACK_JACK) && (getScore() == BLACK_JACK_SCORE));
    }
