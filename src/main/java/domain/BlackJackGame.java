...
public class BlackJackGame {
    ...
    private final static double ONEPOINTFIVE = 1.5;
	...
	while (score.getSum(player.getCards()) <= BLACKJACKNUMBER && morecardinput.getMoreCardOrNot(player.getName())) {
	    getCard(player);
	...
	if (score.getSum(dealer.getCards()) > BLACKJACKNUMBER) {
	    return;
	}
	findWinner();
    }
	...
	for (int i = 0; i < player.length; i++) {
	    ...
	    if (playerscore < BLACKJACKNUMBER && playerscore >= winnerscore) {
	...
	for (int i = 0; i < player.length; i++) {
	    if (score.isBlackJack(score.getSum(player[i].getCards()))) {
		check = true;
		playerMoney[i] = player[i].getBettingMoney() * ONEPOINTFIVE;
